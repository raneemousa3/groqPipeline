"""
Enhanced Groq Pipeline: Advanced Speech → Text → LLM Processing → Intelligent Speech Response

This enhanced pipeline includes:
1. Real-time STT (Deepgram) with better accuracy
2. Advanced LLM processing with context awareness
3. Intelligent prompt engineering
4. Emotion and intent detection
5. Dynamic voice selection
6. Conversation flow management
7. Enhanced TTS with multiple voices
"""
...
# Deepgram WebSocket URL with enhanced parameters
WS_URL = (
...
class EmotionDetector:
    """Detects emotion and intent from user input"""
    
    def __init__(self):
        self.emotion_keywords = {
            "happy": ["happy", "great", "awesome", "excellent", "wonderful", "amazing", "fantastic"],
            "sad": ["sad", "upset", "disappointed", "frustrated", "angry", "mad", "terrible"],
            ...
            "technical": ["code", "programming", "algorithm", "function", "debug", "error", "bug"],
            "creative": ["story", "write", "create", "imagine", "design", "art", "music"]
        }
...
class EnhancedConversationContext:
    """Enhanced conversation context with emotion tracking and conversation flow"""
    ...
    def get_recent_context(self, num_messages: int = 8) -> List[Dict]:
    ...
    def get_conversation_summary(self) -> str:
        ...
        for msg in recent_messages:
            if msg["role"] == "user":
                ...
                if any(word in content for word in ["weather", "temperature", "rain", "sunny"]):
                    topics.append("weather")
                elif any(word in content for word in ["work", "job", "office", "meeting"]):
                    topics.append("work")
                elif any(word in content for word in ["family", "kids", "children", "home"]):
                    topics.append("family")
                elif any(word in content for word in ["food", "eat", "dinner", "lunch", "restaurant"]):
                    topics.append("food")
...
class AdvancedPromptEngineer:
    """Advanced prompt engineering with emotion and context awareness"""
    ...
    def build_enhanced_prompt(self, user_input: str) -> List[Dict]:
        ...
        if conversation_summary != "New conversation started.":
            messages.append({
                "role": "system", 
...
class EnhancedGroqPipeline:
    ...
    def __init__(self):
        ...
        self.prompt_engineer = AdvancedPromptEngineer(self.context)
    ...
    async def deepgram_loop(self) -> None:
        """Main Deepgram WebSocket loop with enhanced processing"""
        async with connect(
            WS_URL,
            ...
            print("🎤 Enhanced Listening Mode... Press CTRL+C to stop")
            print("🧠 Features: Emotion detection, Context awareness, Dynamic voice selection")
    ...
    async def receive_transcripts(self, ws) -> None:
        """Receive and process transcripts with enhanced features"""
